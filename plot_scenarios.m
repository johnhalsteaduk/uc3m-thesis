...
% Total Factor Productivity (TFP) using non-linear Marto damage function
Z_a_sim_no = get_sim(d_no, 'Z_a'); Z_a_lag_no = [get_ss(d_no, 'Z_a'), Z_a_sim_no(1:end-1)];
Z_a_sim_base = get_sim(d_base, 'Z_a'); Z_a_lag_base = [get_ss(d_base, 'Z_a'), Z_a_sim_base(1:end-1)];
calc_tfp_no = (1 - get_sim(d_no,'D') ./ (1 + pi_val*Z_a_lag_no).^nu_D).^(1 - w_k*alpha - w_z*psi_z);
calc_tfp_base = (1 - get_sim(d_base,'D') ./ (1 + pi_val*Z_a_lag_base).^nu_D).^(1 - w_k*alpha - w_z*psi_z);
...
calc_cad = @(data) ([0, diff(get_sim(data, 'q') .* get_sim(data, 'B'))] ./ get_sim(data, 'Y')) * 100;
...
calc_tfp_adapt = (1 - get_sim(d_adapt,'D') ./ (1 + pi_val*Z_a_lag_adapt).^nu_D).^(1 - w_k*alpha - w_z*psi_z);

subplot(1, 3, 1); hold on;
plot(years, pct_dev(d_base, 'Z'), 'b-', 'LineWidth', 1.5);
plot(years, pct_dev(d_adapt, 'Z'), 'r*-', 'LineWidth', 1.5);
